# example-inspec-overlay-profile-against-rhel7-stig-baseline-profile
...
```bash
# How to run 
inspec exec https://github.com/ejaronne/example-inspec-overlay-profile-against-rhel7-stig-baseline-profile/archive/main.tar.gz -t ssh://TARGET_USERNAME:TARGET_PASSWORD@TARGET_IP:TARGET_PORT --sudo --sudo-password=<SUDO_PASSWORD_IF_REQUIRED> --input-file <path_to_your_input_file/name_of_your_input_file.yml> --reporter=cli json:<path_to_your_output_file/name_of_your_output_file.json> 
```

Against a remote target using a pem key with escalated privileges (i.e., InSpec installed on a separate runner host)
```bash
# How to run 
inspec exec https://github.com/ejaronne/example-inspec-overlay-profile-against-rhel7-stig-baseline-profile/archive/main.tar.gz -t ssh://TARGET_USERNAME@TARGET_IP:TARGET_PORT --sudo -i <your_PEM_KEY> --input-file <path_to_your_input_file/name_of_your_input_file.yml> --reporter=cli json:<path_to_your_output_file/name_of_your_output_file.json>  
```

Against a local Red Hat host with escalated privileges (i.e., InSpec installed on the target)
```bash
# How to run
sudo inspec exec https://github.com/ejaronne/example-inspec-overlay-profile-against-rhel7-stig-baseline-profile/archive/main.tar.gz --input-file <path_to_your_input_file/name_of_your_input_file.yml> --reporter=cli json:<path_to_your_output_file/name_of_your_output_file.json> 
```
...
mkdir profiles
cd profiles
git clone https://github.com/ejaronne/example-inspec-overlay-profile-against-rhel7-stig-baseline-profile.git
inspec archive example-inspec-overlay-profile-against-rhel7-stig-baseline-profile
...
```
cd example-inspec-overlay-profile-against-rhel7-stig-baseline-profile
git pull
cd ..
inspec archive example-inspec-overlay-profile-against-rhel7-stig-baseline-profile --overwrite
sudo inspec exec <name of generated archive> --input-file <path_to_your_input_file/name_of_your_input_file.yml> --reporter=cli json:<path_to_your_output_file/name_of_your_output_file.json> 
```
...
To report a bug or feature request, please open an [issue](https://github.com/ejaronne/example-inspec-overlay-profile-against-rhel7-stig-baseline-profile/issues/new).
